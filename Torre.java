public class Torre extends Pedina {
	public Torre(int x, int y, char funzione, int colore) {
	...
	public boolean move(int destX, int destY, Pedina[][]scacchiera){
		...
		if (scacchiera[x][y].funzione=='T') {
			if (x!=destX && y!=destY)
				return false;			
			else if (y==destY) {//controllo verticale, se c'è qualcosa fra T e la dest, return false
			...
			else if (x==destX) { //controllo orizzontale, se c'è qualcosa fra T e la dest, return false
			...
			if (scacchiera[destX][destY]==null){ //Muovo
				...
				if (avversario(x,y,destX,destY,scacchiera)){ // controllo di poterlo mangiare
					...
					scacchiera[destX][destY]=new Torre(x,y,funzione,colore);
					scacchiera[x][y]=null; 
					x=destX;
					y=destY;
					scacchiera[destX][destY]=this;
